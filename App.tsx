...
const App: React.FC = () => {
  ...
  useEffect(() => {
      const init = async () => {
          ...
          // Check active session (Optional for this mode, but good for persistence if we used Auth)
          // Since we are moving to direct table login for students too, this might not auto-login unless we use localStorage
          // For now, we will leave it, but users will likely need to login again.
  ...
  const handleAuthSubmit = async () => {
    ...
    try {
        if (authMode === 'register' && !isStaffPortal) {
            ...
            // We keep using Supabase Auth for registration to handle unique emails and ID generation cleanly
            // loadUserProfile will then insert/sync to the 'students' table
            ...
            if (data.session) {
                ...
                showNotification("¡Registro exitoso!", 'success');
                 ...
                 // Even if session is missing (auto-confirm off), we try to proceed or ask for login
                 // But since we want table-based login next time, we need the record in 'students'.
                 // loadUserProfile usually handles insertion. 
                 // If no session, we can't call loadUserProfile easily if it relies on RLS auth.uid().
                 // However, since we are moving to table-based login, RLS might need adjustment or we just assume registration worked.
                setAuthError('Cuenta creada. Por favor inicia sesión.');
            }

        } else {
            // -- LOGIN (BOTH STAFF AND STUDENTS VIA TABLE) --
            ...
            if (isStaffPortal) {
                ...
                // Student Login: Direct check against the 'students' table (Requested Change)
                ...
                const { data: studentUser, error: studentError } = await supabase
                    ...
                    .select('*')
                    .eq('email', email)
                    .eq('password', password) // Comparing credentials directly
                    .maybeSingle();
                ...
                if (studentUser) {
                    // Success! Log them in manually without auth token
                    // Pass password to keep it synced/available in state
                    await loadUserProfile(studentUser.id, studentUser.email, studentUser.password);
                    setShowAuthModal(false);
